...
#define FLUID_COMMON_ENSURE_BOUNDARY_CONDITION ensure_boundary_condition
#define FLUID_COMMON_HAS_GRID_SIZE 1
#else
#define FLUID_COMMON_ENSURE_BOUNDARY_CONDITION ensureBoundaryCondition
#endif
...
#define st(p) (bool(FLUID_COMMON_ENSURE_BOUNDARY_CONDITION) ? clamp((p), vec2(0), vec2(1)) : fract(p))
...
bool checkBoundary(vec2 uv){
    return bool(FLUID_COMMON_ENSURE_BOUNDARY_CONDITION) && isObstacle(uv);
}
